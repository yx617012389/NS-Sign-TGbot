...
XSERVER_BASE_URL = os.getenv("XSERVER_BASE_URL", "https://secure.xserver.ne.jp")
XSERVER_LOGIN_URL = os.getenv("XSERVER_LOGIN_URL", f"{XSERVER_BASE_URL}/xapanel/login/xserver")
XSERVER_RENEW_URL = os.getenv("XSERVER_RENEW_URL", f"{XSERVER_BASE_URL}/xapanel/xvps/server/freevps/extend/conf")
...
XSERVER_RENEW_CAPTCHA_FIELD = os.getenv("XSERVER_RENEW_CAPTCHA_FIELD", XSERVER_LOGIN_CAPTCHA_FIELD)
...
def extract_captcha_data_url(html: str) -> Optional[str]:
    match = re.search(r'<img[^>]+src=["\'](data:[^"\']+)["\']', html, re.I)
    if match:
...
def data_url_to_image_file(data_url: str) -> Optional[str]:
...
def solve_ocr(data_url: str, max_retries: int = 3) -> Optional[str]:
    if not data_url:
        return None
    headers = {"Content-Type": "text/plain"}
    for attempt in range(1, max_retries + 1):
        try:
            logger.info("🧩 OCR 第 %s 次请求", attempt)
            resp = requests.post(OCR_API_URL, data=data_url, headers=headers, timeout=30)
            if resp.status_code == 200:
                text = resp.text.strip()
                if text and len(text) >= 4:
                    logger.info("✅ OCR 识别成功: %s", text)
                    return text
                logger.warning("⚠️ OCR 返回无效内容: %s", text)
            else:
                logger.warning("⚠️ OCR HTTP 状态异常: %s", resp.status_code)
        except Exception:
...
def renew_xserver(username: str, password: str) -> Dict[str, str]:
    logger.info("🚀 开始 XServer 续期流程: %s", username)
    ok, message, cookie = login_xserver(username, password)
    if not ok:
        logger.error("❌ 登录失败: %s", message)
        return {
            "name": username,
            "result": message,
    ...
    renew_html = ""
    try:
        logger.info("📄 获取续期页面: %s", XSERVER_RENEW_URL)
        pre_resp = session.get(XSERVER_RENEW_URL, timeout=20)
        renew_html = pre_resp.text
        hidden_fields = extract_hidden_inputs(renew_html)
    except Exception:
        hidden_fields = {}

    captcha_data_url = extract_captcha_data_url(renew_html)
    if not captcha_data_url:
        logger.info("🔍 未发现 data URL，尝试抓取验证码图片")
        captcha_url = guess_captcha_url(renew_html, XSERVER_RENEW_URL)
        if not captcha_url:
            return {
                "name": username,
                "result": "🚫 未找到续期验证码图片地址，请配置 XSERVER_CAPTCHA_URL",
                "time": time.strftime("%Y-%m-%d %H:%M:%S"),
                "site_type": "xs",
            }
        ...
        try:
            ...
            captcha_type = captcha_resp.headers.get("Content-Type", "image/png").split(";")[0]
            captcha_b64 = base64.b64encode(captcha_image).decode("utf-8")
            captcha_data_url = f"data:{captcha_type};base64,{captcha_b64}"
        except Exception as exc:
            return {
                "name": username,
                "result": f"🚫 获取续期验证码图片失败: {exc}",
                "time": time.strftime("%Y-%m-%d %H:%M:%S"),
                "site_type": "xs",
            }
    else:
        logger.info("✅ 使用页面内嵌 captcha data URL")

    captcha_text = solve_ocr(captcha_data_url)
    if not captcha_text:
        screenshot_path = data_url_to_image_file(captcha_data_url)
        return {
            "name": username,
            "result": "🚫 续期验证码 OCR 识别失败，请检查 OCR 服务",
            "time": time.strftime("%Y-%m-%d %H:%M:%S"),
            "site_type": "xs",
    ...
    return {
        ...
        "site_type": "xs",
        "cookie": cookie or "",
        "screenshot_path": data_url_to_image_file(captcha_data_url) if result.startswith("🚫") else "",
    }
